...
1. Resolve the target spec from `$ARGUMENTS` (specs live in `specs/` when committed or `.claude/specs/` when
   local — search both):
   - A path or full filename → use it as-is.
   - A bare title → match `{specs,.claude/specs}/*-<title>.md` (specs are prefixed with a date or an issue
     number); also accept a legacy `<dir>/<title>.md`. If several match, list them and ask which.
   - If `$ARGUMENTS` is empty: if the two dirs together hold exactly one spec, use it; otherwise list them and
     ask which.
